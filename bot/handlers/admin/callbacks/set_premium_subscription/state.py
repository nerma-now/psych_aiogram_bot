...
@router.message(SetPremiumState.subscription_plan_id, IsAdmin(), IsSuperadmin())
async def set_premium_state(message: Message, state: FSMContext, database: AsyncSession):
    ...
    if subscription_plan.is_premium:
        await state.clear()

        raise DuplicativeActionException()

    await SubscriptionPlanRepository().update(session=database, instance=subscription_plan, is_premium=True)
