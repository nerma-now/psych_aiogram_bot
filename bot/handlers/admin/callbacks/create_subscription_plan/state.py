...
async def create_subscription_plan_state(
    ...
    if int(message.text) < config.payments.min_amount \
...
@router.message(CreateSubscriptionPlanState.total_classes_monthly, IsAdmin(), IsSuperadmin())
async def create_subscription_plan_state(
    message: Message,
    state: FSMContext,
    ...
    await SubscriptionPlanRepository().add(
        session=database,
        target=SubscriptionPlan(**schema.model_dump())
    )
